...
└──────────┬──────────┘
           │
           │ https://localhost:9310/myfile.txt?Signature=...&Expires=...
           │
    ┌──────▼─────────────────────┐
...
- Clear port separation (9000 = S3, 9310 = CloudFront)
...
- CloudFront: port 9310 (public-facing)
...
- Make request to CloudFront emulator (port 9310)
...
# Manual curl test
curl -v "http://localhost:9310/s3/test-bucket/test-key?Signature=xxx&Expires=xxx&Key-Pair-Id=xxx"
```
...
# Verify
curl http://localhost:9310/health  # Optional health endpoint
```
