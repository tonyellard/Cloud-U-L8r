...
- Ess-Queue-Ess running on `http://localhost:9320`
...
1. Creating queue 'dotnet-example-queue'...
   Queue created: http://localhost:9320/dotnet-example-queue

2. Listing all queues...
   - http://localhost:9320/dotnet-example-queue
...
2. Configure the client to point to your local instance:
   ...
   var config = new AmazonSQSConfig
   {
       ServiceURL = Environment.GetEnvironmentVariable("SQS_ENDPOINT") ?? "http://localhost:9320"
   };
   ```
